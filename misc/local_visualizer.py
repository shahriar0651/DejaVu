"""
This is a modified version of the local_visualizer.py file from the OpenMMLab repository.
/home/username/miniforge3/envs/openmmlab/lib/python3.8/site-packages/mmdet3d/visualization/local_visualizer.py

It is used to visualize the 3D bounding boxes on the point cloud data.
"""
...
class Det3DLocalVisualizer(DetLocalVisualizer):
    ...
    def _initialize_o3d_vis(self, show=True) -> Visualizer:
        ...
        if os.environ.get('DISPLAY', None) is not None and show:
            print("Setting HD window")
            # Set the desired width and height for the image
            image_width = int(2*1920)  # Adjust width as needed
            image_height = int(2*1080)  # Adjust height as needed
            o3d_vis.create_window(width=image_width, height=image_height)
    ...
    def show(self,
        ...
        print("************** Show the drawn point cloud/image.****************")
        ...
        """
        wait_time = -1 #0.01 #-1 #0.01 #TODO: Hasan To destroy figures after 0.01 second (-1 to no destroy)
        ...
        if hasattr(self, '_image'):
            ...
            if drawn_img is None and drawn_img_3d is None:
                ...
                if vis_task == 'multi-modality_det':
                    ...
                    # Allow the figure to be displayed for 1 second
                    plt.pause(1)  # Pause for 1 second

                    # Close the figure
                    plt.close(fig)  # Close the figure after pausing
        ...
        if hasattr(self, 'o3d_vis'):
            ...
            # R = [row[:3] for row in extrinsic_matrix[:3]]  # 3x3 rotation matrix
            # T = [row[3] for row in extrinsic_matrix[:3]]   # Translation vector

            # # Scale the translation vector for better visibility
            # scaling_factor = 0.1  # Adjust this factor based on your scene's scale
            # scaled_T = [t * scaling_factor for t in T]  # Scale the translation vector

            # # Debugging: Print scaled translation
            # print(f"Scaled Camera Position: {scaled_T}")

            # # Set the sensor position to the scaled translation vector
            # sensor_position = scaled_T

            # # Compute the front vector from the rotation matrix
            # # Assuming the camera looks along the negative Z direction
            # camera_front = [-R[0][2], -R[1][2], -R[2][2]]  # Negate the Z-axis direction from R

            # # Normalize the front vector (if necessary)
            # norm = (camera_front[0]**2 + camera_front[1]**2 + camera_front[2]**2)**0.5
            # camera_front = [component / normview_control for component in camera_front]  # Normalize the front vector

            # # Adjust the up vector (it can be the Y-axis or derived from the rotation matrix)
            # camera_up = [0.0, 1.0, 0.0]  # This can also be derived from R if desired

            # # Set the camera parameters
            # self.view_control.set_lookat(sensor_position)  # Look at the sensor position
            # self.view_control.set_front(camera_front)       # Set front based on rotation matrix
            # self.view_control.set_up(camera_up)              # Up direction based on your setup
            # self.view_control.set_zoom(0.5)                  # Adjust zoom as needed

            # # Optional: Set the initial view port if needed
            ...
            """
            # Working
            # Set the sensor position (car's position)
            sensor_position = [0, 0, 0]  # Example sensor position (x, y, z)
            self.view_control.set_lookat(sensor_position)  # Look at the sensor position
            self.view_control.set_front([-0.75, +0.25, 1])        # Looking in the negative Z direction (forward)
            self.view_control.set_up([0.25, 0.25, -1.0])            # Up direction is Y-axis
            self.view_control.set_zoom(0.50)                 # Adjust zoom as needed
            #-----------------
            """
            # sensor_position = [0, 0, 0]  # Example sensor position (x, y, z)
            # self.view_control.set_lookat(sensor_position)  # Look at the sensor position
            ...
            [
                {
                    "boundingbox_max" : [ 19.937000274658203, 67.833000183105469, 2.7460000514984131 ],
                    "boundingbox_min" : [ -42.667999267578125, -0.059999999999999998, -1.9320000410079956 ],
                    "field_of_view" : 60.0,
                    "front" : [ -0.095123497201702878, -0.98360664786531027, 0.15319752790200353 ],
                    "lookat" : [ 1.838462710655407, 15.831096140990947, 0.73259995208314987 ],
                    "up" : [ 0.022821039842921716, 0.15170019692032427, 0.98816306872642368 ],
            ...
            # Set the camera parameters to look from the sensor's perspective
            self.view_control.set_front([ -0.095123497201702878, -0.98360664786531027, 0.15319752790200353])        # Looking in the negative Z direction (forward)
            self.view_control.set_lookat([1.838462710655407, 15.831096140990947, 0.73259995208314987])  # Look at the sensor position
            self.view_control.set_up([.022821039842921716, 0.15170019692032427, 0.98816306872642368])            # Up direction is Y-axis
            self.view_control.set_zoom(0.17999999999999983)                 # Adjust zoom as needed

        #    # Set the sensor position (car's position)
        #     sensor_position = [8.0710000991821289, 38.387499542236327, -0.37050008773803711]  # Example sensor position (x, y, z)
        #     # Set the camera parameters to look from the sensor's perspective
        #     self.view_control.set_lookat(sensor_position)  # Look at the sensor position
        #     self.view_control.set_front([0.0, 0.0, 1.0])        # Looking in the negative Z direction (forward)
        #     self.view_control.set_up([0.0, 1.0, 0.0])            # Up direction is Y-axis
        #     self.view_control.set_zoom(0.70)                 # Adjust zoom as needed
        #     #-----------------
